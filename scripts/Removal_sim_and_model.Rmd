...
# Simulate data
Question: should simulating N from neg binomial, with alpha = pi0 and beta = pcap work out? Is this circular?
```{r model}
# set seed
set.seed(4681)
...
print(fit1, pars = c("b0",  "b1", "sigma_p", "N_total_nb", "N_tot_huggins", "lp__"), digits_summary = 2)
```
...
for(s in 1:(max(data1$site))){
  ...
  N_table[s, 2:6 ] <- round(quantile(rowSums(draws1$N_i[, data1$site == s]), probs = c(0.025, 0.25, 0.5, 0.75, 0.975)), 0)
}
...
for(s in 1:max(data1$site)){
  hist(rowSums(draws1$N_h[, data1$site == s]), 
       nclass = max(rowSums(draws1$N_h[, data1$site == s]))/2, 
       freq = F,
       main = paste0("Site ", s),
       ylab = "Density",
       xlab = expression(hat(N)),
       xlim = c(sum(data1$site == s), max(rowSums(draws1$N_h[, data1$site == s]))))
  abline(v = sum(data1$site == s), col = 'red', lwd = 3)
  abline(v = N[s], col = 'blue', lwd = 3)
}
```
...
for(s in 1:(max(data1$site))){
  ...
  N_table[s, 2:6 ] <- round(quantile(rowSums(draws1$N_h[, data1$site == s]), probs = c(0.025, 0.25, 0.5, 0.75, 0.975)), 0)
}
